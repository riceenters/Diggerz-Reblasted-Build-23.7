...
function patchGameHtmlForBuild239(input) {
  ...
  ['/api/admin/auth','/api/admin/bans','/api/admin/moderate','/api/admin/donations'].forEach(function(apiPath) {
  ...
  // --- Build 23.9 gameplay/content patch ---
  const b239Replacements = [
    ['Build 23.8 native-packet PvP','Build 23.9 native-packet PvP'],
    ['Build 23.8 native-packet Dig+Trade','Build 23.9 native-packet Dig+Trade'],
    ['z-index: 100;">Build 23.8</div>','z-index: 100;">Build 23.9</div>'],
    ['Build 23.8 multiplayer admin tools. Konami sequence + server-authorized admin session required.','Build 23.9 multiplayer admin tools. Konami sequence + server-authorized admin session required.'],
    ['<h2>Build 23.8 — Diggerz Multiplayer</h2>','<h2>Build 23.9 — Diggerz Multiplayer</h2>'],
    ['<span>MP DEBUG 23.8</span>','<span>MP DEBUG 23.9</span>'],
    ["build:'23.8'","build:'23.9'"],
    ['DiggerzService.WEAPON_GROUPS = [\\n        [93],[139],[248],[276],[326],[327],[328],[329],[370],[371]\\n    ];',
     'DiggerzService.WEAPON_GROUPS = [\\n        [81],[93],[139],[248],[276],[326],[327],[328],[329],[370],[371]\\n    ];'],
    ['K._16(a.x, a.y, 600 * b.x, 600 * b.y, l.z39.j35.u41);',
     'K._16(a.x, a.y, a.x + 160 * b.x, a.y + 160 * b.y, l.z39.j35.u41);']
  ];
  for (const pair of b239Replacements) {
    if (html.includes(pair[0])) html = html.split(pair[0]).join(pair[1]);
    else console.warn('[Diggerz 23.9] client patch target not found:', pair[0].slice(0,80));
  }

  // Beta Gun: keep its attack type for authoritative hit logic, but render the
  // recovered blue ray visual instead of the old Beta projectile that can freeze.
  const echoOld = 'packet.R0(attackType | 0);\\n                packet.R8(self.playerId)';
  const echoNew = 'packet.R0((attackType | 0) === 10 ? 4 : (attackType | 0));\\n                packet.R8(self.playerId)';
  if (html.includes(echoOld)) html = html.replace(echoOld, echoNew);

  const peerEchoOld = "packet.R0(m.attackType|0);packet.R8(peer.id)";
  const peerEchoNew = "packet.R0((m.attackType|0)===10?4:(m.attackType|0));packet.R8(peer.id)";
  if (html.includes(peerEchoOld)) html = html.replace(peerEchoOld, peerEchoNew);

  // Trade dupe fix: offered items are escrowed out of inventory. Persist that
  // removal BEFORE the offer reaches the server, and make completion idempotent.
  const tradeOfferOld = "proto.pvpTradeOffer=function(){var tr=this.pvpTrade;if(!tr)return;this.pvpWriteContainer(tr.localGuid,tr.localSlots);this.pvpSend({t:'trade-offer',tradeId:tr.id,offer:tr.localSlots.map(itemCopy)})}";
  const tradeOfferNew = "proto.pvpTradeOffer=function(){var tr=this.pvpTrade;if(!tr)return;this.markDirty();oldSave.call(this,true);this.pvpWriteContainer(tr.localGuid,tr.localSlots);this.pvpSend({t:'trade-offer',tradeId:tr.id,offer:tr.localSlots.map(itemCopy)})}";
  if (html.includes(tradeOfferOld)) html = html.replace(tradeOfferOld, tradeOfferNew);
  else console.warn('[Diggerz 23.9] trade escrow save patch target not found.');

  const tradeCancelOld = "proto.pvpCancelTradeLocal=function(reason){if(!this.pvpTrade)return;this.pvpRestoreTradeItems();this.pvpCloseTradeWindow(true);this.pvpTrade=null;if(reason)this.message('^1Trade cancelled: ^7'+String(reason))}";
  const tradeCancelNew = "proto.pvpCancelTradeLocal=function(reason){if(!this.pvpTrade)return;this.pvpRestoreTradeItems();this.pvpCloseTradeWindow(true);this.pvpTrade=null;this.markDirty();oldSave.call(this,true);if(reason)this.message('^1Trade cancelled: ^7'+String(reason))}";
  if (html.includes(tradeCancelOld)) html = html.replace(tradeCancelOld, tradeCancelNew);

  const tradeCompleteStart = "proto.pvpCompleteTrade=function(receive){var tr=this.pvpTrade;if(!tr)return;var items=";
  const tradeCompleteNew = "proto.pvpCompleteTrade=function(receive){var tr=this.pvpTrade;if(!tr)return;if(!this.pvpCompletedTrades)this.pvpCompletedTrades={};if(this.pvpCompletedTrades[tr.id])return;this.pvpCompletedTrades[tr.id]=1;var items=";
  if (html.includes(tradeCompleteStart)) html = html.replace(tradeCompleteStart, tradeCompleteNew);

  // UPDATE Hat item 395. The inventory uses the game's own bold text as a
  // lightweight icon; the exact supplied artwork is overlaid on equipped heads.
  const poolTail = '379,380,381,382,383,384,385,386,387,388,389,390,391,392,393,394];\\n    // Build 21.15 loot economy.';
  if (html.includes(poolTail)) html = html.replace(poolTail, '379,380,381,382,383,384,385,386,387,388,389,390,391,392,393,394,395];\\n    // Build 21.15 loot economy.');
  const case394 = html.indexOf('            case 394:\\n                b.Init(f.DUALLIGHTSABRE_PNG());');
  const itemDefault = case394 >= 0 ? html.indexOf('            default:\\n                b.Init(f.UNKNOWN_BLOCK_PNG())', case394) : -1;
  if (case394 >= 0 && itemDefault > case394 && !html.includes('            case 395:\\n                b.Init(f.BLANK_PNG());')) {
    const updateItem =
      '            case 395:\\n' +
      '                b.Init(f.BLANK_PNG());\\n' +
      '                b.a4 = 2;\\n' +
      ...
      '                b.c9 = !0;\\n' +
      '                h.O27(b, 0, -34, 1);\\n' +
      '                var updateHatLabel = new ob(0,0,"",q.MAIN_FONT_BIG);\\n' +
      '                updateHatLabel.D7(b);\\n' +
      '                updateHatLabel.E37("^7UPDATE");\\n' +
      '                updateHatLabel.b6 = -2;\\n' +
      '                updateHatLabel.b7 = -3;\\n' +
      '                updateHatLabel.set_local_xScale(updateHatLabel.set_local_yScale(.34));\\n' +
      '                updateHatLabel.B8 = 5;\\n' +
      '                b._9.push(updateHatLabel);\\n' +
      '                break;\\n';
    html = html.slice(0,itemDefault) + updateItem + html.slice(itemDefault);
  } else if (itemDefault < 0) console.warn('[Diggerz 23.9] UPDATE Hat item insertion point not found.');

  // Support page + UPDATE Hat exact-art overlay.
  const supportScript = `<style id="diggerz-23-9-support-style">
...
#diggerz-support239 .panel{width:min(920px,94vw);max-height:90vh;overflow:auto;background:#171717;border:5px solid #f0c22d;box-shadow:0 0 0 4px #111,0 15px 60px #000;padding:22px;display:grid;grid-template-columns:minmax(0,1.35fr) minmax(220px,.8fr);gap:22px}
...
#diggerz-support239 button{font:900 24px Arial Black,Impact,sans-serif;text-transform:uppercase;border:4px solid #111;background:#f0c22d;color:#111;padding:12px 18px;cursor:pointer;box-shadow:0 5px 0 #8a6800}
#diggerz-support239 button:active{transform:translateY(3px);box-shadow:0 2px 0 #8a6800}
#diggerz-support239 .close{position:absolute;right:18px;top:14px;background:#d94a38;color:#fff;font-size:18px;padding:7px 13px;box-shadow:none}
...
function applySecurityHeaders(req, res) {
  ...
  // Build 23.9: allow the Railway-hosted game to be embedded only by itch.io.
...
async function handleAdminApi(req, res, urlPath) {
  ...
  if (urlPath === '/api/admin/donations' && req.method === 'GET') {
    const session = adminSessionForRequest(req); if (!session) { sendApiJson(res,401,{ok:false,error:'admin-auth'}); return true; }
    sendApiJson(res,200,Object.assign({ok:true},publicDonationBoard())); return true;
  }
  if (urlPath === '/api/admin/donations' && req.method === 'POST') {
    ...
    if(action==='remove'){
      const id=String(body&&body.id||''); const before=donations.length;
      ...
      saveDonations(); sendApiJson(res,200,{ok:true}); return true;
    }
    const amount=Math.round(Number(body&&body.amount)*100)/100;
    if(!Number.isFinite(amount)||amount<1||amount>1000){sendApiJson(res,400,{ok:false,error:'invalid-amount'});return true;}
    ...
    donations.push(entry); donations=donations.slice(-5000); saveDonations();
    log(`Admin ${session.role} verified donation ${entry.id}: ${entry.name} ${entry.amount}`);
    sendApiJson(res,200,{ok:true,donation:entry}); return true;
  }
...
const server = http.createServer(async (req, res) => {
  ...
  if (urlPath === '/api/donations') {
    ...
    sendApiJson(res,200,Object.assign({ok:true},publicDonationBoard())); return;
  }
...
+slider.value}
async function loadBoard(){try{var r=await fetch(API,{cache:'no-store'});var j=await r.json();var rows=Array.isArray(j.donations)?j.donations:[];total.textContent='Verified total: 
...
process.on('SIGINT', () => shutdown('SIGINT'));
process.on('SIGTERM', () => shutdown('SIGTERM'));
+Number(j.total||0).toFixed(2).replace(/\\.00$/,'');list.innerHTML='';if(!rows.length){list.innerHTML='<li>No verified donations yet.</li>';return}rows.slice(0,20).forEach(function(d){var li=document.createElement('li');li.textContent=(d.name||'Anonymous')+' — 
...
process.on('SIGINT', () => shutdown('SIGINT'));
process.on('SIGTERM', () => shutdown('SIGTERM'));
+Number(d.amount||0).toFixed(2).replace(/\\.00$/,'');list.appendChild(li)})}catch(e){list.innerHTML='<li>Leaderboard unavailable.</li>'}}
window.DiggerzSupport239={open:function(){box.classList.add('open');box.setAttribute('aria-hidden','false');loadBoard()},close:function(){box.classList.remove('open');box.setAttribute('aria-hidden','true')}};
slider.addEventListener('input',refreshAmount);refreshAmount();
...
document.getElementById('diggerz-support239-donate').onclick=function(){var n=Math.max(1,Math.min(1000,parseInt(slider.value,10)||1));window.open('https://cash.app/
...
process.on('SIGINT', () => shutdown('SIGINT'));
process.on('SIGTERM', () => shutdown('SIGTERM'));
+CASH+'/'+n,'_blank','noopener,noreferrer')};

var layer=document.getElementById('diggerz-hat239-layer'),nodes={};
function headPoint(ent){try{var head=ent&&ent.i33&&ent.i33.f2?ent.i33.f2('head'):null;if(head&&isFinite(head.A7)&&isFinite(head.A8))return{x:head.A7,y:head.A8-10}}catch(e){};if(ent&&isFinite(ent.A7)&&isFinite(ent.A8))return{x:ent.A7,y:ent.A8-28};return null}
function setHat(key,ent,on,scale){var img=nodes[key];if(!on||!ent){if(img)img.style.display='none';return}var p=headPoint(ent);if(!p){if(img)img.style.display='none';return}if(!img){img=document.createElement('img');img.className='diggerz-update-hat239';img.src=HAT_SRC;layer.appendChild(img);nodes[key]=img}img.style.display='block';img.style.left=p.x+'px';img.style.top=(p.y-8)+'px';img.style.transform='translate(-50%,-100%) scale('+(scale||1)+')'}
function tick(){try{var svc=window.q&&q.diggerzService,local=window.l&&l.z39;if(svc&&svc.state)setHat('local',local,(svc.state.appearance&&svc.state.appearance[1]===HAT_ID),.72);var seen={local:true},peers=svc&&svc.pvpPeers||{};for(var k in peers){var peer=peers[k],ap=peer&&peer.info&&peer.info.appearance,ent=svc.pvpEntityForPeer?svc.pvpEntityForPeer(peer):null;seen[k]=true;setHat(k,ent,!!(ap&&ap[1]===HAT_ID),.72)}for(var n in nodes)if(!seen[n]&&n!=='local')nodes[n].style.display='none'}catch(e){}requestAnimationFrame(tick)}requestAnimationFrame(tick);
})();</script>`;
  if (!html.includes('diggerz-build23-9-support-and-hat')) {
    const bodyClose = html.lastIndexOf('</body>');
    html = bodyClose >= 0 ? html.slice(0,bodyClose) + supportScript + html.slice(bodyClose) : html + supportScript;
  }

  // Build 23.9 changelog.
  ...
  if (!html.includes('Build 23.9 - Economy, Beta Gun, Support & UPDATE Hat') && html.includes(marker239)) {
    const section239 = '<hr><a name="Build 23.9"></a> <h2>Build 23.9 - Economy, Beta Gun, Support &amp; UPDATE Hat</h2> <ul> <li>Closed the multiplayer trade reload/duplicate-item race by persisting trade escrow before offers are sent and ignoring duplicate completion packets.</li> <li>Fixed the Beta Gun firing freeze, added it to common mining weapon drops, and kept it tradable.</li> <li>Added the UPDATE Hat (item 395) with its own player-head placement so existing hats are unchanged.</li> <li>Added the Support Diggerz donation board with a $1-$1000 Cash App link and verified supporter leaderboard.</li> <li>Updated active multiplayer build identifiers to 23.9.</li> </ul> ';
